...
{
  echo "Checking if OPENCD already present in ${WORK_FOLDER_PATH}/${OPENOCD_SRC_FOLDER_NAME}"
  if [ ! -d "${WORK_FOLDER_PATH}/${OPENOCD_SRC_FOLDER_NAME}" ]
  then
    (
      echo "No, then download it"
      xbb_activate

      # <MICROSEMI>
      # Pull SoftConsole openocd source tarball from ftp.actel.com rather
      # than from GME github for the moment.
      ...
      cd "${WORK_FOLDER_PATH}"
      curl -L "${OPENOCD_GIT_URL}" -o "${OPENOCD_SRC_FOLDER_NAME}.tar.gz"
      echo "call tar"
      tar -xzvf "${OPENOCD_SRC_FOLDER_NAME}.tar.gz"
      cd "${OPENOCD_SRC_FOLDER_NAME}"
      git submodule update --init --recursive --remote
...
#     config_options+=("--docdir=${APP_PREFIX_DOC}")
      config_options+=("--enable-microsemi-mods")
# </MICROSEMI>            
