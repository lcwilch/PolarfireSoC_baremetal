...
# <MICROSEMI>
BRANDING=${BRANDING:-"${DISTRO_UC_NAME} OpenOCD (Microchip SoftConsole build)"}
# currently the most upto date sources are on the margaret FTP together with the build script archive and the finished build artifacts
OPENOCD_GIT_URL=${OPENOCD_GIT_URL:-"ftp://ftp.actel.com/incoming/tommy/openocd/openocd-softconsole-src.tar.gz"}   
OPENOCD_SRC_FOLDER_NAME=${OPENOCD_SRC_FOLDER_NAME:-"openocd-softconsole-src"}
# </MICROSEMI>
