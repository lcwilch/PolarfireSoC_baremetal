...
function prepare_xbb_extras()
{
  ...
  then
    ...
    # XBB_CXXFLAGS+=" -O2"
    # XBB_LDFLAGS+=" -O2"
  # Microsemi mods, pump the speeds, we support 64-bit only, it's tricky to find older system than core2 and be 64-bit as well
    if [ "${TARGET_PLATFORM}" == "linux" ]
    then
      XBB_CFLAGS+=" -Ofast -flto -march=core2"
      XBB_CXXFLAGS+=" -Ofast -flto -march=core2"
      XBB_LDFLAGS+=" -Ofast -flto -march=core2"
      # XBB_CFLAGS+=" -O2 -march=core2"
      # XBB_CXXFLAGS+=" -O2 -march=core2"
      # XBB_LDFLAGS+=" -O2 -march=core2"
    else
      # Windows will fail with linker optimalization (flto) enabled
      XBB_CFLAGS+=" -Ofast -march=core2"
      XBB_CXXFLAGS+=" -Ofast -march=core2"
      XBB_LDFLAGS+=" -Ofast -march=core2"
    fi
  fi
...
function prepare_app_libraries()
{
  ...
  (
    ...
    then
      pwd
      echo "striping binary ${app_path}"
      strip_binary "${app_path}"
    fi

# <MICROSEMI>
    do_copy_microsemi_runtime_dependencies
# </MICROSEMI>

    echo "depending on the platform ${TARGET_PLATFORM} the following will be done "
    if [ "${TARGET_PLATFORM}" == "linux" ]
    then
      echo
