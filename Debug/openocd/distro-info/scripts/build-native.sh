...
HIDAPI_VERSION="0.8.0-rc1-no-win-report_id2"
...
then
  ...
  do_libusb1 "${LIBUSB1_VERSION}"
  if [ "${TARGET_PLATFORM}" == "win32" ]
  then
    do_libusb_w32 "${LIBUSB_W32_VERSION}"
  else
    do_libusb0 "${LIBUSB0_VERSION}"
  fi

  do_libftdi "${LIBFTDI_VERSION}"

  do_libiconv "${LIBICONV_VERSION}"

  do_hidapi "${HIDAPI_VERSION}"
